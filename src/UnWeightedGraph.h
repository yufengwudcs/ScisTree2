...
{
public:
    GraphVertex() {value = id = 0; visited = false;}
    GraphVertex(int id1) : visited(false) {id = id1;}
    GraphVertex(int id1, int val) : visited(false) {id = id1; value = val;}
    GraphVertex(const GraphVertex &rhs) {value = rhs.value; id = rhs.id; visited = rhs.visited;}
...
{
public:
    GraphEdge() {vid1 = -1; vid2 = -1; value = -1;}
    GraphEdge(int id1, int id2) {vid1 = id1; vid2 = id2;}
    GraphEdge(int id1, int id2, int v) {vid1 = id1; vid2 = id2; value = v;}
    GraphEdge(const GraphEdge &rhs) {vid1 = rhs.vid1; vid2 = rhs.vid2; value = rhs.value; label = rhs.label;}

    void GetVertexIDs(int &v1, int &v2) {v1 = vid1; v2 = vid2;}
    int GetValue() {return value;}
    void SetValue(int v) {value = v;}
    string GetLabel() const {return label;}
    void SetLabel(string lbl) {label = lbl;}
...
public:
    ...
    void GetAllNodesIds(std::set<int> &setIds);
...
public:
    ...
    // more algorithms for graph: return two parts with node ids in two
    int GetMaxCut(set<int> &cutPart1, set<int> &cutPart2);
