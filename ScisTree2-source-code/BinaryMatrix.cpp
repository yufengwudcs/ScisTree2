...
bool BinaryMatrix :: IsColNonInformative(int c) const
{
...
bool BinaryMatrix :: IsCladeCompatibleWithSite( const set<int> &clade, int c ) const
{
    bool f00 = false;
    bool f01 = false;
    bool f10 = false;
    ...
    vector<int> vecClade( GetRowNum() );
    for(unsigned int i=0; i<vecClade.size(); ++i)
    {
        vecClade[i] = 0;
    }
    for(set<int> :: const_iterator it = clade.begin(); it != clade.end(); ++it)
    {
        vecClade[*it] = 1;
    }
    ...
    for(unsigned int i=0; i<rowsArray.size(); ++i)
    {
        if(rowsArray[i][c] == 0 && vecClade[i] == 0)
        {
            f00 = true;
        }
        if(rowsArray[i][c] == 0 && vecClade[i] == 1)
        {
            f01 = true;
        }
        if(rowsArray[i][c] == 1 && vecClade[i] == 0)
        {
            f10 = true;
        }
        if(rowsArray[i][c] == 1 && vecClade[i] == 1)
        {
            f11 = true;
        }
    }
...
bool BinaryMatrix :: IsCompatible( int c1, int c2) const
{
	bool f00 = false;
	bool f01 = false;
	bool f10 = false;
...
int BinaryMatrix :: ComputeMinRecombWeight( int rowIndex, vector<pair<int,int> > *pListSegs )
{
...
//Dump();
    ...
    vector<pair<int,int> > listSegsInf;     // <ending position, row that gives best match>
    ...
    for(int curpos=0; curpos < GetColNum(); ++curpos)
    {
        ...
        if(sint.size() == 0)
        {
            ...
            // save the ending point and which row gives
            YW_ASSERT_INFO( curpos > 0, "Canont be at start" );
            YW_ASSERT_INFO( lastTrackRows.size() > 0, "Last tracking rows: cannot be empty" );
            pair<int,int> pp(curpos-1, *lastTrackRows.begin() );
            listSegsInf.push_back(pp);
...
//            PopulateSetWithInterval( lastTrackRows, 0, mat.size() - 1 );
        ...
        if(curpos == GetColNum()-1)
        {
            //
            YW_ASSERT_INFO( lastTrackRows.size() > 0, "Cannot be empty" );
            pair<int,int> pp( curpos,  *lastTrackRows.begin() );
            listSegsInf.push_back(pp);
        }
    }
...
int BinaryMatrix :: GetDiffSitesForTwoRowsRegion(int r1, int r2, int c1, int c2) const
{
    int res = 0;
    for(int c=c1; c<=c2; ++c)
    {
...
double BinaryMatrix :: CalcAvePairRowsDiffRegion(const set<int> &setRows, int c1, int c2) const
{
    double res = 0.0;
    int numPairs = 0;
    for(set<int> :: const_iterator it1 = setRows.begin(); it1 != setRows.end(); ++it1)
    {
        int r1 = *it1;
        set<int> :: const_iterator it2 = it1;
        ++it2;
        for(; it2 != setRows.end(); ++it2 )
        {
            int r2 = *it2;
            ++numPairs;
            res += GetDiffSitesForTwoRowsRegion(r1,r2, c1, c2);
        }
    }
    return res/( (c2-c1+1)* numPairs);
}
...
void ConsBinMatrixFromSplits( BinaryMatrix &matBin, int numRows, const set<set<int> > &setClades )
{
    matBin.SetSize(  numRows, setClades.size() );
    int col = 0;
    for(set<set<int> > :: iterator it = setClades.begin(); it != setClades.end(); ++it)
    {
        ...
        for(int r=0; r<numRows; ++r)
        {
            ...
            matBin.SetValAt(r, col, val);
        }
        ++col;
    }
#if 0
cout << "Matrix: for clades: numRows: " << numRows << ", clades: \n";
for(set<set<int> > :: iterator it = setClades.begin(); it != setClades.end(); ++it)
{
DumpIntSet(*it);
}
