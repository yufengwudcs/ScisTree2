...
double GetLogSumOfLogs2(const vector<double> &listLogs)
{
    if( listLogs.size() == 0)
    {
        // nothing to process, return smallest
        return -1.0e100;
    }
    // given a list of log terms, compute the sum of prob (need to take exp)
    // and express the sum in the log again
...
double GetLogSumOfLogs3(const vector<double> &listLogs)
{
    if( listLogs.size() == 0)
    {
        // nothing to process, return smallest
...
double GetLogDiffOfTwo(double logv1, double logv2)
{
...
bool ReadLinesFromFile(const char *fname, vector<string> &listLines)
{
    // data input
    ifstream inFile(fname);
    if(!inFile)
    ...
    while (std::getline(inFile, str))
    {
        ...
        listLines.push_back(str);
    }
    return true;
}

bool ReadListSeqsFromFile(const char *fname, vector<vector<int> > &listSeqs)
{
    // file contains sequences e.g. 010001111
    // read into sequence list; assume 0/1/2
    ifstream inFile(fname);
    if(!inFile)
    {
        cout << "Can not open "<< fname <<endl;
    ...
    while( inFile.eof() == false )
    {
        ...
        if( strlen(buffer) > 0 )
        {
            ...
            for(int i=0; i<strlen(buffer); ++i)
            {
                ...
                seq.push_back(allele);
            }
            listSeqs.push_back(seq);
        }
    }
...
void AdjustVector(vector<int> &vec, int diff)
{
    // adjust each value in the vec by diff
