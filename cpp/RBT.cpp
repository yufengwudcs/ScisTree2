...
void RBTNode :: OutputNodeGML( ofstream &outFile)
{
	...
	{
		name[0] = 'v'; 
		snprintf(&name[1],99, "%d", GetLeafId() );
	}
	else
	{
		name[0] = ' ';
...
//cout << "Here...\n";
    ...
    for(int i=0; i<numTNodes; ++i)
    {
        YW_ASSERT_INFO( (listNodeLeftChild[i] >=0 && listNodeRightChild[i] >=0) 
            || (listNodeLeftChild[i] <0 && listNodeRightChild[i] <0), "WRONG-a" );
    ...
    vector< double > &listEdgeDist  )
{
    ...
    while( true )
    {
        ...
        else
        {
            ...
            YW_ASSERT_INFO( mapNodeToIndices.find(plc) != mapNodeToIndices.end(), "WRONG-b"  );
            YW_ASSERT_INFO( mapNodeToIndices.find(prc) != mapNodeToIndices.end(), "WRONG-c"  );
